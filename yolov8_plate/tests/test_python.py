...
@pytest.mark.skipif(not ONLINE, reason='environment is offline')
def test_data_utils():
    """Test utility functions in ultralytics_plate/data/utils.py."""
    from ultralytics.data.utils import HUBDatasetStats, autosplit
    from ultralytics.utils.downloads import zip_directory

    # from ultralytics_plate.utils.files import WorkingDirectory
...
def test_utils_checks():
    ...
    checks.check_imshow()
    checks.check_version('ultralytics_plate', '8.0.0')
    checks.print_args()
...
def test_utils_patches_torch_save():
    ...
    with patch('ultralytics_plate.utils.patches._torch_save', new=mock):
